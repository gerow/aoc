...
func (s *state) visOccupied(v vec2) int {
	var occ int
	for _, d := range []vec2{
		{1, 0},
		{1, 1},
	...
	loop:
		for {
			...
			if l[0] < 0 || l[1] < 0 || l[0] >= s.width || l[1] >= s.height {
				// If we get to an edge without seeing any seat then we don't see an occupied seat.
				break
			}
			switch s.at(l) {
			case occupied:
	...
	for y := 0; y < s.height; y++ {
		for x := 0; x < s.width; x++ {
			...
			case floor:
				continue
			case empty:
				if prev.visOccupied(v) == 0 {
					s.set(v, occupied)
					changed = true
				}
			case occupied:
				if prev.visOccupied(v) >= 5 {
					s.set(v, empty)
					changed = true
				}
			}
