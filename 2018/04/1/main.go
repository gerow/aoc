...
func main() {
	...
	for _, g := range guards {
		if g.MinutesAsleep > sleepiest.MinutesAsleep {
			sleepiest = g
		}
	}
	log.Printf("sleepiest guard is %+v", sleepiest)

	var totalMinute int
	var bestMinute int
	for m, v := range sleepiest.TimesAsleep {
		if v > totalMinute {
			totalMinute = v
			bestMinute = m
		}
	...
	log.Printf("guard %d slept a total of %d minutes, best minute was %d during which they slept %d total minutes",
		sleepiest.ID, sleepiest.MinutesAsleep, bestMinute, totalMinute)
	fmt.Print(int(sleepiest.ID) * bestMinute)
}
